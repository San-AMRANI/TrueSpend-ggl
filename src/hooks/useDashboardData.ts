...
export function useDashboardData(token: string | null) {
  ...
  const handleSettleDebt = async (debtId: string, amount: number) => {
    try {
      await dashboardService.settleDebt(debtId, amount, token);
      await fetchData();
    } catch (e) {
  ...
  const handleSaveSettings = async (newPayday: number, newBuffer: number) => {
    setIsSaving(true);
    try {
      await dashboardService.updateSettings({ payday: newPayday, emergencyBuffer: newBuffer }, token);
      await fetchData();
