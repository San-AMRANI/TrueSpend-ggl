...
import { CategoryBudget, KPI, Transaction, Debt, DashboardTab, Payroll, Goal, Subscription, DetectedSubscription, ImpulseItem, ImpulseStats, FireProfile } from '../types';
...
export function useDashboardData(token: string | null) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [kpiData, txData, debtData, settingsData, budgetData, payrollData, contextData, goalData, subData, impulseData, impulseStatsData, fireProfileData] = await Promise.all([
