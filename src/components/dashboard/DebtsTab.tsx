      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {debts.map((debt) => (
              <div key={debt.id} className="flex flex-col border-b border-gray-100 dark:border-gray-800 pb-4 sm:pb-3 pt-2 sm:pt-0 last:border-0 last:pb-0">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-900 dark:text-gray-100">{debt.contactName}</p>
                      <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                        <span>{format(new Date(debt.createdAt), 'MMM d, yyyy')}</span>
                        <span>•</span>
                        <span>{debt.type === 'Receivable' ? 'Owes you' : 'You owe'}</span>
                        <span>•</span>
                        <span className={debt.status === 'Pending' ? 'text-amber-600 dark:text-amber-400 font-medium' : 'text-green-600 dark:text-green-400 font-medium'}>
                          {debt.status}
                        </span>
                      </div>
                    </div>
                ...
                {debt.settlements && debt.settlements.length > 0 && (
                  <div className="mt-4 pl-12 sm:pl-12">
                    <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 flex items-center gap-1 mb-2">
                      <History className="h-3 w-3" /> Settlement History
                    </div>
                    <div className="space-y-1">
                      {debt.settlements.map((s) => (
                        <div key={s.id} className="flex items-center justify-between text-xs text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/50 px-2 py-1 rounded">
                          <span>{format(new Date(s.createdAt), 'MMM d, yyyy - h:mm a')}</span>
                          <span className="font-medium text-gray-900 dark:text-gray-100">{parseFloat(s.amount).toFixed(2)} MAD</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {debts.length === 0 && <p className="py-4 text-center text-sm text-gray-500 dark:text-gray-400">No debts or splits recorded.</p>}
          </div>
        </CardContent>
      </Card>
