    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {showFilters && <div className="space-y-5 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 p-4">
          ...
          <div className="grid gap-4 lg:grid-cols-3"><div><p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Type</p><div className="flex flex-wrap gap-3">{(['Expense', 'Income', 'Transfer'] as Transaction['type'][]).map((type) => <label key={type} className="flex items-center gap-1.5 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={filters.types?.includes(type)} onChange={() => update({ types: toggle(filters.types, type) })} />{type}</label>)}</div></div><div><p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Wallet</p><div className="flex gap-3">{(['Bank', 'Cash'] as Transaction['walletId'][]).map((wallet) => <label key={wallet} className="flex items-center gap-1.5 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={filters.wallets?.includes(wallet)} onChange={() => update({ wallets: toggle(filters.wallets, wallet) })} />{wallet}</label>)}</div></div><div className="grid grid-cols-2 gap-3"><label className="space-y-1 text-sm font-medium text-gray-700 dark:text-gray-300">Min amount<Input min="0" type="number" value={filters.minAmount ?? ''} onChange={(event) => update({ minAmount: event.target.value === '' ? undefined : Number(event.target.value) })} /></label><label className="space-y-1 text-sm font-medium text-gray-700 dark:text-gray-300">Max amount<Input min="0" type="number" value={filters.maxAmount ?? ''} onChange={(event) => update({ maxAmount: event.target.value === '' ? undefined : Number(event.target.value) })} /></label></div></div>
