...
export const DashboardNav: React.FC<DashboardNavProps> = ({ activeTab, setActiveTab }) => {
  ...
  return <>
    ...
    <nav aria-label="Dashboard sections" onTouchStart={(event) => { touchStartY.current = event.touches[0].clientY; if (isExpanded) cancelCollapse(); }} onTouchEnd={handleTouchEnd} className="fixed inset-x-0 bottom-0 z-50 m-0 mb-0 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/95 px-2 pb-0 pt-1 shadow-[0_-4px_16px_rgba(0,0,0,0.08)] backdrop-blur md:hidden">
