...
export const DashboardNav: React.FC<DashboardNavProps> = ({ activeTab, setActiveTab }) => {
  ...
  return <>
    <nav aria-label="Dashboard sections" className="sticky top-0 z-30 hidden rounded-lg border-b border-gray-200 bg-white p-1 shadow-xs md:block">
      <div className="flex space-x-2 overflow-x-auto no-scrollbar scroll-smooth">{tabs.map((tab) => { const Icon = tab.icon; const isActive = activeTab === tab.id; return <button key={tab.id} type="button" aria-current={isActive ? 'page' : undefined} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 whitespace-nowrap rounded-md px-4 py-2 text-sm font-medium transition-all ${isActive ? 'bg-gray-900 text-white shadow-xs' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}><Icon className="h-4 w-4" /><span>{tab.label}</span></button>; })}</div>
    </nav>

    <nav aria-label="Dashboard sections" onTouchStart={(event) => { touchStartY.current = event.touches[0].clientY; if (isExpanded) cancelCollapse(); }} onTouchEnd={handleTouchEnd} className="fixed inset-x-0 bottom-0 z-50 border-t border-gray-200 bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] pt-1 shadow-[0_-4px_16px_rgba(0,0,0,0.08)] backdrop-blur md:hidden">
