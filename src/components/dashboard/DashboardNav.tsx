...
import { LayoutDashboard, ArrowRightLeft, Users, BarChart2, FileText, Settings, Calculator, WalletCards, CalendarDays, ChevronDown, ChevronUp, Bot, FileBarChart, TrendingUp, Map, Target, Repeat, ShieldCheck, Flame } from 'lucide-react';
  ...
  const renderTab = (tab: typeof tabs[number], compact = false) => {
    ...
    return (
      <button
        ...
        className={`flex min-h-14 flex-col items-center justify-center gap-1 rounded-lg px-1 text-[10px] font-medium leading-none transition-colors ${
          isActive ? 'bg-gray-100 dark:bg-gray-800 text-gray-950' : 'text-gray-500 dark:text-gray-400 active:bg-gray-100 dark:bg-gray-800'
        }`}
      >
        <Icon className={`h-5 w-5 ${isActive ? 'stroke-[2.5]' : ''}`} />
        <span className="max-w-full truncate">{compact ? tab.shortLabel : tab.label}</span>
      </button>
    );
  };
  ...
  return (
    <nav
      ...
      onTouchStart={(event) => {
        ...
        if (isExpanded) cancelCollapse();
      }}
      onTouchEnd={handleTouchEnd}
      className={`fixed inset-x-0 bottom-0 z-50 m-0 mb-0 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/95 px-2 pb-0 pt-1 shadow-[0_-4px_16px_rgba(0,0,0,0.08)] backdrop-blur md:hidden transition-transform duration-200 ${
        activeTab === 'chat' ? 'translate-y-full pointer-events-none' : 'translate-y-0'
      }`}
    >
      <div className="mx-auto max-w-lg">
        <button
          ...
          aria-label={isExpanded ? 'Collapse all navigation pages' : 'Expand all navigation pages'}
          onClick={() => (isExpanded ? close() : open())}
          className="flex h-5 w-full items-center justify-center text-gray-400"
        >
          <span className="mr-1 h-1 w-9 rounded-full bg-gray-300" />
          {isExpanded ? <ChevronDown className="h-3 w-3" /> : <ChevronUp className="h-3 w-3" />}
        </button>
        <div
          className={`grid grid-cols-4 overflow-hidden transition-[max-height,opacity] duration-300 ease-out ${
            isExpanded ? 'max-h-52 opacity-100' : 'pointer-events-none max-h-0 opacity-0'
          }`}
        >
        ...
        <div
          className={`grid grid-cols-4 overflow-hidden transition-[max-height,opacity] duration-300 ease-out ${
            isExpanded ? 'pointer-events-none max-h-0 opacity-0' : 'max-h-16 opacity-100'
          }`}
        >
          {tabs.filter((tab) => quickTabIds.includes(tab.id)).map((tab) => renderTab(tab, true))}
        </div>
      </div>
    </nav>
  );
