...
export const usersRelations = relations(users, ({ many, one }) => ({
  ...
  resilienceProfile: one(resilienceProfiles),
...
export const resilienceProfiles = pgTable('resilience_profiles', {
  id: uuid('id').defaultRandom().primaryKey(),
  userId: uuid('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull().unique(),
  emergencyTargetMonths: integer('emergency_target_months').default(6).notNull(),
  stressJobLossMonths: integer('stress_job_loss_months').default(3).notNull(),
  stressEmergencyExpense: decimal('stress_emergency_expense').default('2500').notNull(),
  stressInflationRate: decimal('stress_inflation_rate').default('12.0').notNull(),
  essentialExpensesRatio: decimal('essential_expenses_ratio').default('60.0').notNull(),
  customEssentialMonthly: decimal('custom_essential_monthly'),
  microLeakThreshold: decimal('micro_leak_threshold').default('20.0').notNull(),
  notes: text('notes'),
