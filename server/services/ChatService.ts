...
/**
 * Parses raw OCR text from a receipt/ticket into a structured proposal.
 * Returns a confidence score (0-100) so the caller can decide whether
 * to trust the result.
 */
export async function parseReceiptText(ocrText: string): Promise<ReceiptProposal> {
  const empty: ReceiptProposal = {
    amount: null,
    currency: null,
    merchant: null,
    date: null,
  ...
  // Truncate so we don't blow up the context window with garbage from a 4K image
  const cleaned = ocrText.replace(/\s{2,}/g, ' ').trim().slice(0, 4000);

  const systemPrompt = `You are a receipt-parsing assistant. Extract structured information from the OCR text of a receipt or ticket. Return ONLY a raw JSON object — no markdown fences, no backticks, no preamble.

Schema:
{
  "amount": number | null,            // total amount paid (number only, no currency symbol)
  "currency": string | null,          // ISO code or symbol if visible, else null
  "merchant": string | null,          // store / restaurant / brand name
  "date": string | null,              // transaction date in YYYY-MM-DD, or null
  "category": string | null,          // MUST be one of the categories below, or null
  "wallet": "Bank" | "Cash" | null,   // default null when ambiguous
  "notes": string | null,             // one short line summarizing the items, or null
  "confidence": number                // 0-100, your overall confidence in this parse
}
...
Rules:
- If the OCR is mostly noise or you cannot find a total, set amount=null and confidence<30.
- Use today's date (most recent plausible date) if no date is visible.
- Confidence should reflect how readable the receipt was AND how certain you are about the amount and merchant.
- NEVER invent values. If unsure, use null and lower the confidence.`;

  const userPrompt = `OCR text from a receipt:\n\n"""\n${cleaned}\n"""`;
  ...
  for (const m of freeModels) {
    if (candidates.length >= 3) break;
    if (!candidates.includes(m) && m !== 'openrouter/free') candidates.push(m);
  }
  if (!candidates.includes('openrouter/free')) candidates.push('openrouter/free');
  ...
  try {
    const response = await fetchWithTimeout(
      'https://openrouter.ai/api/v1/chat/completions',
      {
        ...
        headers: {
          Authorization: `Bearer ${apiKey}`,
          'HTTP-Referer': 'https://truespend.app',
          'X-Title': 'TrueSpend Receipt Parser',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          models: openRouterModels,
    ...
    if (!response.ok) {
      ...
      console.error(`[parseReceiptText] OpenRouter failed (${response.status}): ${detail}`);
      return empty;
    }
    ...
    if (!raw.trim()) {
      console.error('[parseReceiptText] Empty response from OpenRouter');
      return empty;
    }
    ...
    try {
      ...
      console.error('[parseReceiptText] Could not parse JSON:', raw.slice(0, 300));
      return empty;
    }
    ...
    const amountNum =
      typeof parsed.amount === 'number'
        ? parsed.amount
        : parsed.amount && !Number.isNaN(parseFloat(parsed.amount))
    ...
    const wallet =
      parsed.wallet === 'Bank' || parsed.wallet === 'Cash' ? parsed.wallet : null;

    const category =
      typeof parsed.category === 'string' && RECEIPT_CATEGORIES.includes(parsed.category)
        ? parsed.category
        : null;
    ...
    console.error('[parseReceiptText] Unexpected error:', error);
    return empty;
  }
}
